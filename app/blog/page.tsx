...
const BLOGS: Blog[] = [
  {
    ...
    date: "2025-05-26",
    excerpt:
      "A short essay introducing the geometric intuition behind arithmetic.",
    tags: ["Mathematics", "Geometry", "Arithmetic"],
    content: `# Toward a Dimensional Theory of Arithmetic
...
When we add two numbers in this framework, we're not just combining scalars—we're combining geometric forms.

### Conclusion

This essay connects arithmetic and geometry.`,
  },
  {
    id: "b2",
    slug: "the-nature-of-time-energy-and-light",
    ...
    date: "2025-11-06",
    excerpt:
      "Exploring how time emerges from energy exchange, light as the messenger of change, and entropy as the arrow of time.",
    tags: ["Physics", "Thermodynamics", "Relativity"],
    content: `# The Nature of Time, Energy, and Light

## Prelude

What is time? This essay explores time as an emergent property of energy exchange.`,
  },
];
...
function renderMarkdown(text: string = ""): string {
  return text
    ...
    .map((line, idx) => {
      if (line.startsWith("# ")) {
        return `<h1 class="text-3xl font-bold mt-6 mb-3">${line.slice(
          2
        )}</h1>`;
      }
      if (line.startsWith("## ")) {
        return `<h2 class="text-2xl font-bold mt-5 mb-2">${line.slice(
          3
        )}</h2>`;
      }
      if (line.startsWith("### ")) {
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6 max-w-4xl mx-auto">
      ...
      <div className="space-y-6">
        {BLOGS.map((b) => (
          <article key={b.id} className="bg-white p-6 rounded shadow">
            <div className="flex items-center gap-2 text-sm text-gray-500">
              <Calendar size={14} />
              {b.date}
            </div>
            <h2 className="text-2xl font-semibold mt-2">{b.title}</h2>
            <p className="mt-2 text-gray-600">{b.excerpt}</p>

            <a
              href={`#blog/${b.slug}`}
              className="inline-block mt-3 text-teal-600"
            >
              Read →
            </a>
          </article>
...
function BlogDetailPage({ slug }: { slug: string }) {
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6 max-w-3xl mx-auto">
      <a href="/" className="text-teal-600 flex items-center gap-2 mb-4">
...
/* -------------------- App -------------------- */
export default function BlogApp() {
  const [currentPage, setCurrentPage] = useState<"index" | "detail">("index");
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      ...
      const blogLink = target.closest(
        'a[href^="#blog/"]'
      ) as HTMLAnchorElement | null;

      if (blogLink) {
        e.preventDefault();
        handleNavigate(blogLink.href.split("#blog/")[1]);
      }
